...
export default defineConfig({
  // Test files live in saas/tests/ (Node module resolution needs them inside
  ...
  use: {
    /* Base URL — localhost dev server, which resolves to Staging Winery via
       DEFAULT_TENANT_ID (see MaintenanceNotes.md §4). Never point this at
       master/production. */
  ...
  /* Reuses a dev server you already have running (per README.md); starts one
     otherwise. Never runs `next build`/`next start` here — this must stay on
     the dev server against the dev DB, not a production-mode build. */
  webServer: {
    command: 'npm run dev',
