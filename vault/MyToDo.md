...
- [x] **Dev/prod environments** — ✅ BUILT 2026-07-23 as #79 (Option A+B as recommended below, plus prisma migrate switch). See `Plan-DevProdEnvironments.md`. Historical brainstorm kept for reference:
...
- [ ] **Nikalas Marani payment system (Flitt) — planned 2026-07-29, ready to build** — trigger decision now made (the existing booking "Confirm" and wine "Place Reservation" buttons become purchase buttons; per-tenant toggle so tenants without online payment keep today's reservation-only flow). Full build plan, phases, blockers and files: [[Plan-OnlinePayment.md]]. Research it builds on: [[MigrationNotes.md]] → "Old site source (pre-migration)" section.
  - ~~**Blocking first step:** confirm Flitt merchant account `4056054` is still enabled.~~ **Done 2026-07-29 — account is live**, credentials valid, and the signature algorithm is already validated as a side effect. Phases 1–4 are unblocked.
  - **Needs Max's answer before phases 5/7** (not before 1–4): wine-order receipts (`WineOrder` has no email column), abandoned-checkout policy, whether unconfigured pricing should fail loudly, and whether hidden-price company bookings may be charged. Listed in full in the plan's §7.
  - Also outstanding: rotate the merchant password (plaintext in the old PHP, now also in Downloads), and get Flitt merchant-portal access from the client for refunds/transaction visibility.
