...
- ~~**~6s page renders are NORMAL for this app on BOTH prod and staging**~~ — **NO LONGER TRUE (fixed 2026-07-29).** This was never "normal", it was a misconfiguration: the Vercel functions ran in `iad1` (Washington DC) while both Supabase projects live in `eu-central-1` (Frankfurt), so every DB round trip crossed the Atlantic. Fixed by pinning the region in `saas/vercel.json`. Pages now respond in **~0.4s** on prod and staging alike. If you ever see multi-second page loads again, **check `X-Vercel-Id` first** — it should read `fra1::fra1::…`; a second segment of `iad1` means the region pin was lost. Full detail: [[Plan-Performance]], numbers: [[Perf-Baseline-2026-07-29]].
