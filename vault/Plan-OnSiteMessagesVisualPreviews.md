...
| **B** | New Company popup → `NewCompanyPopupView.tsx` | High — tightly coupled to `BookingForm.tsx`'s state (~15-20 props: `companyId`, `newCoStatus`, handlers, etc.); this exact file already caused 2 production crashes this cycle ([[KnownBugs]] #33/#34) from a careless split | ⬜ Not started |
...
Not started. Needs a separate go-ahead given the risk profile above. Rough shape when it happens:
`NewCompanyPopupView.tsx` takes `variant: 'withBooking' | 'noBooking' | 'sent' | 'error'` plus the
6 content strings already wired via `mc()`, plus the 4 form field values + onChange handlers (or,
for the preview caller, static sample values + no-op handlers) and a submit handler (or none, in
preview mode — the button just becomes inert like Payment Result's `preview` link). Admin side
gets a 4-way pill switcher mirroring Payment Result's pattern.
