...
| 64 | **Editing an already-paid order silently detaches its displayed total from the amount actually charged — and nothing anywhere can tell.** `updateOrderEnhanced` (`app/actions/orders.ts`) recomputes `Order.totalPrice` (and the rate snapshots) whenever an admin edits guest counts/extras, but never once references `tx.payment` — confirmed by reading the function in full, then proving it live (Plan-PaymentE2ETesting Chunk 6, 2026-09-25). `Payment.amount` is written exactly once, at `startCheckout()` time, and after that is only ever *read* (`settle.ts`'s amount-equality gate, checked once, against the *original* callback — never re-checked against a later edit). **Live-verified, not just read from the code:** a real individual booking (Tasting + Lunch, 4 guests) was taken through an actual Flitt settlement on Staging Winery's permanent test merchant — `Payment{ provider: 'flitt', method: 'CARD', status: 'approved' }`, `amount` = 48000 tetri (480₾), agreeing exactly with `Order.totalPrice` at that point. The order's own admin detail page was then used to raise the party size and the Tasting+Lunch split from 4 to 6 (the ordinary "add two more guests" edit, at the *same* per-person rate the order was already sold at — the most sympathetic version of this edit, not an edge case) and saved. Afterward: `Order.totalPrice` = 72000 tetri (720₾) on **every single UI surface checked** — the admin orders table, the order's own detail page ("Order Total": 720.00₾), the CSV export, and a freshly re-sent invoice email (720 stated, sent to the guest's real address) — while a direct read of the same `Payment` row shows `amount` still exactly 48000 tetri, `status='approved'`, `settledAt` unchanged, byte-for-byte identical to before the edit. **A real customer whose card was charged ₾480 now has an order that says, everywhere an admin or the customer themselves could look, that ₾720 is owed/was paid — with no way to notice the ₾240 discrepancy short of directly querying the database.** There is no payments-list screen anywhere in this app (confirmed again while investigating this — the only UI-adjacent read of `Payment` is indirect, via `Order.paidAt`), no reconciliation check, no warning banner, nothing that ever compares these two numbers. Real-world impact: an admin re-sending an invoice or reading the order detail page after any post-payment edit (a very ordinary thing to do — adding a guest, a masterclass line, or an extra to a booking that already has a deposit or full payment on file) would confidently tell a customer the wrong amount is owed, in either direction (undercharging or overcharging, symmetrically — the bug is "the two facts stop being the same," not that the total always moves one way), and neither the admin nor the customer has any way to catch it, since every screen agrees with every other screen, just not with what the gateway actually processed. **Not fixed here, per [[ClaudeInstructions]] Rule 8** — this chunk's job was to test and document, not to patch `updateOrderEnhanced`. Live-verified reproducible test: `saas/tests/tier5-payment-e2e/payment-edit-after-payment.spec.ts`; full write-up: `playwright/notes/17-payment-edit-after-payment.md`. **Design spike (2026-09-25), see [[Plan-PaymentE2ETesting]]:** the proposed two-part fix (lock price fields once paid; charge a legitimate top-up as a second, independent `Payment` row) has three real, previously-unknown obstacles on the second-payment side, all fixable — `recordManualPayment`'s `hasLivePayment()` silently swallows a second manual payment on an already-paid order (confirmed live: a real 24000-tetri top-up call created zero new rows, no error); `createCheckout`/`startCheckout` pass the Order's own id straight through as Flitt's `order_id`, and Flitt's live API rejects a second checkout that reuses it (`Duplicate order <id> for merchant 1549901` — needs a distinct Flitt-facing id per attempt); and `addOrderExtra` has no post-payment gate at all today, so adding an extra to a paid order already silently reprices `totalPrice` the same way #64 describes, just via a different field. Also found: once a second `Payment` row exists, the order detail page's "Paid · <method>" label (`app/admin/(panel)/orders/[id]/page.tsx`) reads only the single newest settled payment, so a card payment topped up with cash/bank transfer at the door would silently relabel the *original* charge's method. The lock-price-fields half has no such obstacle — `updateOrderEnhanced` and the extras action are already fully separate code paths. **Fix designed and chunked, not yet built: [[Plan-PostPaymentExtras]]** (2026-09-25) — the full agreed design (lock price fields, extras become a visible balance-due instead of a silent reprice, collect the balance as a genuinely separate `Payment` row by manual entry or a real card-link checkout, fix the mislabeling above) as a 7-chunk sequential build plan. | Payments / Orders | 🔴 Open — fix designed, not built |
| 65 | **A second (or later) real settlement on an order still in booking-stage `NEW` silently drags `Order.paidAt` forward to the LATEST payment's time, not the first.** `settle.ts`'s post-settlement write is `tx.order.updateMany({ where: { id: payment.orderId, stage: 'NEW' }, data: { paidAt: settledAt, abandonedAt: null } })` — gated on booking **stage**, not on whether `paidAt` was already set. The comment beside it explains the *stage* guard's real purpose ("an order a human already moved on... must not have its fulfilment dragged backwards by a late callback") — a genuinely different concern from "has this order already been paid once," which nothing in this write actually checks. Found live, not by inspection, during [[Plan-PostPaymentExtras]] Chunk 4's own required verification (2026-09-25): a throwaway order was paid manually (Bank transfer), left in stage `NEW` throughout (never advanced — an ordinary case, not contrived), then given two separate real card-link top-ups through Flitt's permanent test merchant. Each of the two real settlements rewrote `Order.paidAt` to its own settlement time — first to the ₾100 top-up's timestamp, then again to the ₾50 top-up's timestamp, confirmed by direct DB read after each — moving the order's "first paid" fact forward by minutes on every real callback, with the original manual `Payment` row itself completely untouched throughout (only `Order.paidAt` drifts, not the ledger). The design spike behind Chunk 4 ([[Plan-PaymentE2ETesting]], "Design spike (2026-09-25)" §3) tested a second settlement too and reported "no misfire observed" — true only because that spike deliberately advanced the order's stage to `CONFIRMED` first, specifically to exercise this exact guard, which is what made it correctly block the second write *there*. Same lesson bug #60's own writeup names directly: a guard has to gate on the one fact that must never change twice — here that fact is `paidAt` itself, not `stage`, which protects a real but different concern that just happens to share one `updateMany` call with this one. **Consequence:** any order that collects a second real payment while still sitting in stage `NEW` (realistic for a winery that doesn't rigorously advance the stage picker on every booking) shows a "first paid" date/time later than it actually was, on every screen or report that reads `Order.paidAt`. **Not fixed here** — `settle.ts` is the shared, heavily-hardened settlement file behind bug #60's whole saga, and this fix (likely adding `paidAt: null` to the `where` clause alongside `stage: 'NEW'`) deserves the same dedicated care and its own live verification, not a rushed addition folded into the chunk that found it. Full narrative in [[Plan-PostPaymentExtras]] Chunk 4's Result section. | Payments / Orders | 🔴 Open |
