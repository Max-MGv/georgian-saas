...
| 8 | Testing | 🚧 | **Automated: done.** 64 tests green — 18 flow (wine settlement, cross-tenant rejection, declined→payment_failed, processing left alone), 9 Payment RLS, 37 signature. Module-off regression verified live for both forms. **Done by Max on staging:** module toggled on for Staging Winery, Card Payments credentials entered and confirmed masked, both booking and wine checkouts reached Flitt's real payment page with correct amounts, one wine checkout abandoned and the resulting "Awaiting Payment" card + count badge + "Mark as paid" verified live (moved the order back into the normal flow correctly). **Done 2026-07-29 (this entry):** production migration + RLS applied and verified (§1a); `staging` → `master` merged and pushed (`429a16e`) — **production is now running this code, with the module off for Nikalas Marani**. **Still remaining, all needing Max — see §9a for the full list:** Resend domain verification (hard go-live blocker, §8a); one real *completed* payment on staging with a refund via the Flitt portal — no genuine inbound Flitt callback has ever round-tripped, only self-generated test callbacks; then, once both of those are clear, enabling the module for Nikalas Marani for real. |
...
## 9a. What's left (as of 2026-07-29) — the single up-to-date list

Everything else in this document is history. This is the current state.

**Shipped:** all 7 build phases + the wine-order payment-state fix, on `master` (`429a16e`) and live in production. **Nothing customer-visible has changed** — `modulesOnlinePayment` is `false` for Nikalas Marani and every other tenant, so this is a dormant deploy.

**Verified so far:** 64 automated tests (18 flow, 9 Payment RLS, 37 signature); module-off regression checked live for both forms; on staging, Max walked the module-on path for both booking and wine checkout through to Flitt's real payment page with correct amounts, confirmed the secret key is never shown once saved, and verified an abandoned wine checkout produces a correct "Awaiting Payment" card with a working "Mark as paid" recovery.

**Not yet done — three items, in this order, all needing Max:**

1. **Verify the sending domain in Resend, then flip `isDomainVerified` to `true`** in both `saas/lib/emails/bookingConfirmation.ts` and `saas/lib/emails/wineOrderReceipt.ts` (and move `from` off `onboarding@resend.dev`). **Hard blocker** — until this is done, a paying customer's receipt goes to Max's inbox instead of theirs. Full reasoning: §8a.
2. **One real, low-value payment completed end-to-end on staging, then refunded via the Flitt merchant portal.** Every test so far — automated and manual — has stopped at or forged the callback; no genuine inbound Flitt callback has ever round-tripped through `settle.ts`. This is the only step that proves it does. Needs Flitt portal access, which Max does not yet have confirmed (also needed to eventually rotate the merchant password — §2).
