...
| 79 | **[DRAFT]** Company soft-auth / access codes — one-time code per company (admin can regenerate or hardcode); entered on booking/wine order form to pre-fill company data; company profile extended with tax code, address, etc.; admin-editable | Admin + Public site | 📝 Draft | ❌ No | ❌ No |
| 80 | **[DRAFT]** Editable social/contact icon links — admin page to update URLs for Instagram, Facebook, phone, email etc. without touching code | Admin | 📝 Draft | ❌ No | ❌ No |
| 81 | **[DRAFT]** Forgot password for admins — reset link sent to Max or company rep's email; Supabase reset flow | Admin | 📝 Draft | ❌ No | ❌ No |
| 82 | **[DRAFT]** Feature flags panel — internal panel for Max to toggle features on/off (e.g. wine orders tab, image backgrounds, masterclass section) | Admin | 📝 Draft | ❌ No | ❌ No |
| 83 | **[DRAFT]** Printable daily booking sheet — employee-facing print/export view; bookings grouped by day per row with all relevant info | Admin | 📝 Draft | ❌ No | ❌ No |
| 84 | **[DRAFT]** Printable wine packing sheet — employee-facing print/export view; total bottles per wine across upcoming orders with per-company box distribution breakdown | Admin | 📝 Draft | ❌ No | ❌ No |
| 85 | **[DRAFT]** Wine orders — total upcoming bottles banner — top-of-page summary showing total bottles per wine across all non-delivered orders for easy employee packing | Admin | 📝 Draft | ❌ No | ❌ No |
